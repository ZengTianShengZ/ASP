...
public class BottomTagView extends View {
	...
	public BottomTagView(Context context, AttributeSet attrs,
		...
		for (int i = 0; i < num; i++) {
			int i1 = ta.getIndex(i);
			if (i1 == R.styleable.BottomTagView_tagicon) {
				BitmapDrawable drawable = (BitmapDrawable) ta
						.getDrawable(R.styleable.BottomTagView_tagicon);
				mIconBitmap = drawable.getBitmap();

			} else if (i1 == R.styleable.BottomTagView_tagtext) {
				mText = ta.getString(R.styleable.BottomTagView_tagtext);

			} else if (i1 == R.styleable.BottomTagView_tagsize) {
				mTextsize = (int) ta
						.getDimension(R.styleable.BottomTagView_tagsize,
								10);

			} else if (i1 == R.styleable.BottomTagView_tagcolor) {
				mColor = ta.getInt(
						R.styleable.BottomTagView_tagcolor,
						0x00ff00);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int IconWide = Math.min(getMeasuredHeight() - getPaddingTop()
